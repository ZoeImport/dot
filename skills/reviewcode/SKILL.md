...
- **svc 返回值约定**：
  - `err == nil` 时 `res` 不得为 `nil`。
  - **内部错误（DAO 失败、外部调用失败等）必须 `return nil, err`**，禁止将 InternalError 嵌入 `res.Code` 后 `return res, nil`——这会污染职责边界，导致 api 层无法区分"业务拒绝"和"系统异常"。
  - **业务拒绝**（如"记录不存在"、"无权限"等用户可感知错误）可将 `res.Code = ErrCode_BadRequest/NoPermission` 后 `return res, nil`。
  - 禁止在 svc 内造带 HTTP 码的自定义 `error` 类型。

### G-07a api 层必须检查 svc 返回的 Go error [Error]

- **禁止**用 `_` 丢弃 svc 调用的 error 返回：`res, _ := svc.Xxx(ctx, req)` 是错误写法。
- 若 svc 返回 `nil, err`（内部异常），api 层必须：
  1. 记 `logs.ErrorContextf(ctx, "[Handler] svc failed: %v", err)`
  2. 设 `resp.Code = errcode.ErrCode_InternalError`
  3. `return`
- **正确示例**：
  ```go
  res, err := svcbook.GenBookSyncTasks(ctx, req)
  if err != nil {
      logs.ErrorContextf(ctx, "[GenBookSyncTasks] svc failed: %v", err)
...
- **错误示例**：
  ```go
  res, _ := svcbook.ListBooks(ctx, req)  // ❌ nil panic 风险
  resp.Code = res.Code
  ```
- Review 时：凡 svc 函数签名返回 `error`，调用处若出现 `_, _` 或 `res, _` 一律报 Error。
...
### G-08a api 层禁止跳过 svc 直接调业务实现 [Error]

- `internal/apis` 只允许调用本 app 的 `services/svc*` 包，**禁止**直接调用：
  - 其他 app 的 `services/devtask`、`services/worker*` 等非 `svc*` 命名的实现包
  - 跨 app 的 svc 包（应通过本 app 的 svc 层做桥接）
- **识别信号**：`internal/apis` 的 import 中出现非 `svc*` 前缀的 `services/` 包，即为违规。
- **原因**：配置加载、参数合并、错误映射等业务逻辑若放在 api 层，导致 api 臃肿且无法复用，且 devtask 等包可能引入与 api 无关的依赖。
- **正确做法**：在本 app 的 `services/svcXxx/` 中新建函数，由 api 调用 svc，再由 svc 调用 devtask/其他实现。
- **错误示例**：
  ```go
  // ❌ api 直接调 devtask，同时把配置加载和参数合并写在 api 层
  import "apps/dotteacher/services/devtask"
  func GenBookSyncTasks(ctx, req, resp) {
      cfg := settings.GetYaml(...)      // 配置加载不该在 api
      baseURL := firstNonEmpty(req.BaseURL, cfg.BaseURL)  // 合并逻辑不该在 api
      devtask.GenBookSyncTasks(ctx, ...)
  }
  // ✅ api 只做校验 + 调 svc
  func GenBookSyncTasks(ctx, req, resp) {
      res, err := svcbook.GenBookSyncTasks(ctx, req)
...
### G-22a 禁止使用 apiobj.Filter 动态字段查询，必须使用 Cond 类型字段 [Error]

- **禁止**在 svc 层或 DAO 层构建 `[]apiobj.Filter{...}` 并通过 `BaseCond.Filters` 或 `Cond.Filters` 传递查询条件。`apiobj.Filter.Field` 是 `string`，字段名无类型约束，IDE 无法补全，改表名或字段名时无法被静态工具发现。
- **正确做法**：在对应 `XxxCond` struct 中添加明确类型的字段（如 `Subject string`、`Period string`），在 `BuildCondition` 中对这些字段做 `if cond.Subject != "" { db = db.Where(...) }` 处理，svc 层直接赋值 `cond.Subject = req.Subject`。
- **清理**：如果某个 `XxxCond` 的 `Filters []apiobj.Filter` 字段已无调用方，同步删除该字段和 `BuildCondition` 中对应的 switch/for 处理块。

**错误示例**：
```go
// ❌ svc 层：魔法字符串 "subject"，字段改名不会报错
filters = append(filters, apiobj.Filter{Field: "subject", Value: []string{req.Subject}})
cond := &teacher.QuestionBookCond{BaseCond: teacher.BaseCond{Filters: filters}}

// ❌ DAO BuildCondition：switch 魔法字符串
for _, filter := range cond.BaseCond.Filters {
    switch filter.Field {
    case "subject": db = db.Where("subject IN (?)", filter.Value)
    }
}
```
...
// ✅ BuildCondition 直接用字段
if cond.Subject != "" { db = db.Where(dao.TableName()+".subject = ?", cond.Subject) }

// ✅ svc 层直接赋值
cond := &teacher.QuestionBookCond{Subject: req.Subject, Period: req.Period}
```
...
## Ponytail 审阅（覆盖所有代码段）[必须执行]

**每次 review 必须对所有审阅到的代码段额外过一遍 Ponytail 梯阶**，独立于 G-xx 规则体系，报告在单独的 Ponytail 审阅节中。

### 梯阶检查（按序，第一个命中即停）

| 梯阶 | 问题 | 判断标准 |
|------|------|---------|
| P-1 YAGNI | 这段代码需要存在吗？ | 只用一次、调用方可直接内联、或功能已有现成实现 |
| P-2 已有实现 | stdlib / 已引入依赖 / 项目内已有函数能做到吗？ | grep 当前代码库，有则直接复用 |
| P-3 内联 | 能收成一行或两行内联逻辑吗？ | 函数体 ≤ 3 行且只有 1 个调用处 → 应内联 |
| P-4 放错了层 | 这段逻辑放对地方了吗？ | 工具函数在 api 层、业务逻辑在 model 层、配置处理在 handler 层 → 均属放错层 |
| P-5 过度抽象 | 抽象是否有多于一个实现/多于两个调用方？ | 接口只有一个实现、factory 只产出一种产品、config 值从不变化 → 删除抽象 |

### 典型 Ponytail 违规样例

```go
// ❌ P-1 + P-3：firstNonEmpty 只用了 3 次，逻辑是简单的 if == ""
...
if auth == ""    { auth = cfg.Auth }

// ❌ P-4：配置加载放在 api 层
func MyHandler(ctx, req, resp) {
    cfg := settings.GetYaml(...)   // 不该在 api 层
    ...
}

// ❌ P-2：项目已有 pkgs/utils.ToMap，自己写了一遍 map 构建
sourceMap := make(map[uint]string)
for _, item := range list { sourceMap[item.ID] = item.Name }  // 重复轮子

// ❌ P-5：接口只有一个实现，且永远不会有第二个
type BookSyncer interface { Sync(ctx) error }
type ExambankBookSyncer struct{}
func (s *ExambankBookSyncer) Sync(ctx) error { ... }
```
...
| 梯阶 | 行号 | 代码片段 | 建议 |
|------|------|---------|------|
| P-1 YAGNI | 92–98 | `func firstNonEmpty(...)` | 只有 3 处调用，内联 `if == ""` 即可，删除函数 |
| P-4 放错层 | 38–44 | `settings.GetYaml(...)` | 配置加载属 svc 职责，移出 api handler |
```

**Ponytail 不报告**：已有充分调用方（≥ 3 处）的公共函数、跨包共享的工具、性能/安全必要的抽象。
