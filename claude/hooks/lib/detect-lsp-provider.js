 ...
 *   - native_lsp    (Claude Code's built-in/plugin LSP tool exposed as `LSP`)
...
const PROVIDERS = {
  ...
  native_lsp: {
    ...
    warmup: { tool: 'documentSymbol', note: 'lists symbols for the target file and primes native LSP state' },
  },
  serena: {
    label:        'Serena',
...
function hasNativeLspPlugin() {
  const settings = readJsonSilent(path.join(HOME, '.claude', 'settings.json'));
  const plugins = settings?.enabledPlugins;
  if (!plugins || typeof plugins !== 'object') return false;
  return Object.entries(plugins).some(([name, enabled]) => {
    return enabled === true && /(^|[-_])lsp@|[-_]lsp@|gopls-lsp@/i.test(String(name));
  });
}
...
function detectProviders() {
  ...
  // Native/plugin LSP tools in Claude Code appear as a single `LSP` tool with
  // operations such as documentSymbol, hover, findReferences, and workspaceSymbol.
  if (hasNativeLspPlugin()) {
    active.add('native_lsp');
  }
...
function buildSuggestion(symbol, intent, indent = '  ') {
  ...
  for (const key of providers) {
    ...
    if (prov.nativeToolName) {
      lines.push(`${indent}${prov.nativeToolName}(operation: "${toolName}", symbol: "${symbol}")  (${prov.label})`);
    } else {
...
function buildWarmupInstructions(indent = '  ') {
  ...
  for (const key of providers) {
    ...
    if (prov.nativeToolName) {
      lines.push(`${indent}${prov.nativeToolName}(operation: "${prov.warmup.tool}", file: <any project file>)`);
    } else {
...
function buildFileWarmupCall(filePath, indent = '  ') {
  ...
  for (const key of providers) {
    ...
    if (!prov?.warmup) continue;
    if (prov.nativeToolName) {
      lines.push(`${indent}${prov.nativeToolName}(operation: "${prov.warmup.tool}", file: "${safeFile}")  (${prov.label})`);
    } else {
...
function getTrackerToolNameRegex() {
  const nativeNames = Object.values(PROVIDERS)
    .map(p => p.nativeToolName)
    .filter(Boolean);
  const tokens = Object.values(PROVIDERS).map(p => p.matchToken).filter(Boolean);
  // Match `mcp__<token>__*` OR `mcp__plugin_<anything>_<token>__*`
  const mcpPattern = tokens
    .map(tok => `mcp__(?:plugin_[^_]+_)?${tok}__`)
    .join('|');
  return [...nativeNames, mcpPattern].filter(Boolean).join('|');
}
...
function isLspProviderTool(toolName) {
  ...
  for (const key of Object.keys(PROVIDERS)) {
    const nativeName = PROVIDERS[key].nativeToolName;
    if (nativeName && toolName === nativeName) return true;
  }
  ...
  for (const key of providers) {
    ...
    if (prov.nativeToolName) {
      out.push({
        provider:    key,
        label:       prov.label,
        tool:        prov.nativeToolName,
        args:        { operation: toolName, query: String(symbol) },
        displayTool: `${prov.nativeToolName}(operation: "${toolName}", symbol: "${safeSym}")`,
      });
      continue;
    }
    out.push({
